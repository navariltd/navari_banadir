...
def execute(filters=None):
	...
	if filters.doc_type == "All":
		...
		return Analytics(filters).run(filters)
...
def convert_currency_columns(data, filters):
	...
	while current_date <= to_date:
		...
		quarter_field = f'quarter_{quarter}_{year}'
		quarters.add(quarter_field)

		# Add yearly field
		years.add(f'{year}')
		
		# Add weekly field
		week = get_week_of_year(current_date)
		week_field = f'week_{week}_{year}'
		weeks.add(week_field)

		# Increment current date by one week
		current_date += timedelta(weeks=1)
		
	while current_month_date <= to_date:
		month_year_field = current_month_date.strftime('%b_%Y').lower()
		months.add(month_year_field)
		# Move to the first day of the next month
		current_month_date = current_month_date.replace(day=1) + timedelta(days=32)
		current_month_date = current_month_date.replace(day=1)
	
	# Combine weeks, quarters, and years into a sorted list
	currency_fields = sorted(weeks.union(months).union(quarters).union(years))
 
	currency_fields.append('total')  # Add 'total' at the end
