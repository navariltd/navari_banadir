...
class StockBalanceReport:
	...
	def get_columns(self, filters):
     ...
     "precision": 2,
				...
				{"label": _(f"In Value({presentation_currency})"), "fieldname": "in_val", "fieldtype": "Float",
					"precision": 2, "width": 80},
				{
					"label": _("Out Qty"),
					"fieldname": "out_qty",
					"fieldtype": "Float",
     "precision": 2,
				...
				{"label": _(f"Out Value({presentation_currency})"), "fieldname": "out_val", "fieldtype": "Float",
					"precision": 2, "width": 80},
				{
...
#The convert function is inverted hence I had to invert the values to get the right figures
	def convert_currency_fields(self, data, filters):
		...
		for entry in data:
			entry['bal_val'] = convert(entry.get('bal_val', 0), from_currency, to_currency, date)
