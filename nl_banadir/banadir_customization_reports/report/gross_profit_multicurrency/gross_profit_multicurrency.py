...
def get_data_when_not_grouped_by_invoice(
	gross_profit_data, filters, group_wise_columns, data
):
...
class GrossProfitGenerator:
	...
	def process(self):
		...
		grouped_by_invoice = (
			True if self.filters.get("group_by") == "Invoice" else False
		)
					...
					if product_bundle:
						for packed_item in product_bundle:
							if (
								...
								and packed_item.get("parent_detail_docname")
								== row.item_row
							):
					...
					if row.indent == 1.0:
						if (
							...
							for returned_item_row in returned_item_rows:
								...
								row.base_amount += flt(
									returned_item_row.base_amount,
									self.currency_precision,
								)
							row.buying_amount = flt(
								flt(row.qty) * flt(row.buying_rate),
								self.currency_precision,
							)
					...
					if i == 0:
						...
						if self.filters.get("group_by") == "Sales Person":
							new_row.allocated_amount += flt(
								row.allocated_amount, self.currency_precision
							)
				new_row = self.set_average_rate(new_row)
				self.grouped_data.append(new_row)

	def set_average_based_on_payment_term_portion(
		self, new_row, row, invoice_portion, aggr=False
	):
	...
	def is_not_invoice_row(self, row):
		return (
			self.filters.get("group_by") == "Invoice" and row.indent != 0.0
		) or self.filters.get("group_by") != "Invoice"
	...
	def calculate_buying_amount_from_sle(
		self, row, my_sle, parenttype, parent, item_row, item_code
	):
		for i, sle in enumerate(my_sle):
			...
			if (
				...
				previous_stock_value = (
					len(my_sle) > i + 1 and flt(my_sle[i + 1].stock_value) or 0.0
				)

				if previous_stock_value:
					return (
						abs(previous_stock_value - flt(sle.stock_value))
						* flt(row.qty)
						/ abs(flt(sle.qty))
					)
				else:
	...
	def get_last_purchase_rate(self, item_code, row):
		...
		query = (
			...
			.select(
				...
				purchase_invoice_item.base_rate
				/ purchase_invoice_item.conversion_factor,
			)
		...
		query = query.orderby(
			purchase_invoice.posting_date, order=frappe.qb.desc
		).limit(1)
	...
	def get_delivery_notes(self):
		...
		if self.si_list:
			...
			delivery_notes = (
				...
				.where(
					(dni.docstatus == 1) & (dni.against_sales_invoice.isin(invoices))
				)
	...
	def get_bundle_items(self, product_bundle):
		return frappe.get_all(
			"Product Bundle Item",
			filters={"parent": product_bundle.item_code},
