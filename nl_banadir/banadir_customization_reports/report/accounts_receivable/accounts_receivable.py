...
def execute(filters=None):
    ...
    if filters.get("presentation_currency"):
        args["presentation_currency"] = filters.get("presentation_currency")
...
class ReceivablePayableReport:
    ...
    def set_defaults(self):
        ...
        self.dr_or_cr = (
            "debit" if self.filters.account_type == "Receivable" else "credit"
        )
    ...
    def init_voucher_balance(self):
        ...
        if self.filters.get("group_by_party") and not self.filters.get(
            "in_party_currency"
        ):
    ...
    def get_voucher_balance(self, ple):
        if self.filters.get("sales_person"):
            if not (
                ...
                or ple.against_voucher_no
                in self.sales_person_records.get("Sales Invoice", [])
            ):
                return
        ...
        if not row:
            ...
            if self.filters.get("ignore_accounts"):
                row = self.voucher_balance.get(
                    (ple.voucher_type, ple.voucher_no, ple.party)
                )
            else:
                row = self.voucher_balance.get(
        ...
        for _key, row in self.voucher_balance.items():
            ...
            if self.filters.get("for_revaluation_journals"):
                if (abs(row.outstanding) > 0.0 / 10**self.currency_precision) or (
                    abs(row.outstanding_in_account_currency)
                    > 0.0 / 10**self.currency_precision
                ):
                    must_consider = True
            else:
                if (abs(row.outstanding) > 1.0 / 10**self.currency_precision) and (
                    (
                        abs(row.outstanding_in_account_currency)
                        > 1.0 / 10**self.currency_precision
                    )
        ...
        if self.filters.get("presentation_currency"):
            ...
            if not conversion_rate:
                ...
                for entry in self.data:
                    entry['currency'] = self.filters.get("presentation_currency")
                    entry['account_currency'] = self.filters.get("presentation_currency")
                    entry['invoiced'] = convert(entry.get('invoiced', 0), from_currency, to_currency, conversion_data[1])
                    entry['paid'] = convert(entry.get('paid', 0), from_currency, to_currency, conversion_data[1])
                    entry['credit_note'] = convert(entry.get('credit_note', 0), from_currency, to_currency, conversion_data[1])
                    entry['outstanding'] = convert(entry.get('outstanding', 0), from_currency, to_currency, conversion_data[1])
                    entry['invoiced_in_account_currency'] = convert(entry.get('invoiced_in_account_currency', 0), from_currency, to_currency, conversion_data[1])
                    entry['paid_in_account_currency'] = convert(entry.get('paid_in_account_currency', 0), from_currency, to_currency, conversion_data[1])
                    entry['credit_note_in_account_currency'] = convert(entry.get('credit_note_in_account_currency', 0), from_currency, to_currency, conversion_data[1])
                    entry['outstanding_in_account_currency'] = convert(entry.get('outstanding_in_account_currency', 0), from_currency, to_currency, conversion_data[1])
                    entry['invoice_grand_total'] = convert(entry.get('invoice_grand_total', 0), from_currency, to_currency, conversion_data[1])
                    entry['range1'] = convert(entry.get('range1', 0), from_currency, to_currency, conversion_data[1])
                    entry['range2'] = convert(entry.get('range2', 0), from_currency, to_currency, conversion_data[1])
                    entry['range3'] = convert(entry.get('range3', 0), from_currency, to_currency, conversion_data[1])
                    entry['range4'] = convert(entry.get('range4', 0), from_currency, to_currency, conversion_data[1])
                    entry['range5'] = convert(entry.get('range5', 0), from_currency, to_currency, conversion_data[1])
                    entry['total_due'] = convert(entry.get('total_due', 0), from_currency, to_currency, conversion_data[1])
    ...
    def build_delivery_note_map(self):
        if self.invoices and self.filters.show_delivery_notes:
            ...
            for d in dn_against_si:
                self.delivery_notes.setdefault(d.against_sales_invoice, set()).add(
                    d.parent
                )
    ...
    def get_invoice_details(self):
        ...
        if self.account_type == "Receivable":
            ...
            if self.filters.show_sales_person:
                ...
                for d in sales_team:
                    self.invoice_details.setdefault(d.parent, {}).setdefault(
                        "sales_team", []
                    ).append(d.sales_person)
    ...
    def allocate_extra_payments_or_credits(self, row):
        ...
        if additional_row:
            additional_row.outstanding = (
                additional_row.invoiced
                - additional_row.paid
                - additional_row.credit_note
            )
        ...
        for future in self.future_payments.get((row.voucher_no, row.party), []):
            ...
            if row.remaining_balance != 0 and future.get(future_amount_field):
                if future.get(future_amount_field) > row.outstanding:
                    ...
                    future[future_amount_field] = (
                        future.get(future_amount_field) - row.outstanding
                    )
    ...
    def get_return_entries(self):
        doctype = (
            "Sales Invoice" if self.account_type == "Receivable" else "Purchase Invoice"
        )
        filters = {
            "is_return": 1,
            "docstatus": 1,
    ...
    def get_ple_entries(self):
        ...
        if self.filters.get("show_remarks"):
            if remarks_length := frappe.db.get_single_value(
                ...
                query = query.select(
                    Substring(ple.remarks, 1, remarks_length).as_("remarks")
                )
            else:
    ...
    def add_common_filters(self):
        ...
        if self.filters.finance_book:
            self.qb_selection_filter.append(
                self.ple.finance_book == self.filters.finance_book
            )

        if self.filters.get("party_type"):
            self.qb_selection_filter.append(
                self.filters.party_type == self.ple.party_type
            )
        ...
        if self.filters.party_account:
            self.qb_selection_filter.append(
                self.ple.account == self.filters.party_account
            )
        else:
        ...
        if self.filters.get("sales_partner"):
            self.qb_selection_filter.append(
                self.ple.party.isin(
                    ...
                    .select(self.customer.name)
                    .where(
                        self.customer.default_sales_partner
                        == self.filters.get("sales_partner")
                    )
                )
            )
        ...
        if self.filters.get("supplier_group"):
            self.qb_selection_filter.append(
                self.ple.party.isin(
                    ...
                    .where(
                        supplier.supplier_group == self.filters.get("supplier_group")
                    )
                )
            )
    ...
    def get_hierarchical_filters(self, doctype, key):
        ...
        groups = (
            qb.from_(doc).select(doc.name).where((doc.lft >= lft) & (doc.rgt <= rgt))
        )
        customers = (
            qb.from_(customer).select(customer.name).where(customer[key].isin(groups))
        )
            ...
            for dimension in accounting_dimensions:
                if self.filters.get(dimension.fieldname):
                    if frappe.get_cached_value(
                        ...
                        self.qb_selection_filter.append(
                            self.ple[dimension.fieldname].isin(
    ...
    def get_columns(self):
        ...
        if self.filters.based_on_payment_terms:
            ...
            self.add_column(
                label=_("Invoice Grand Total"), fieldname="invoice_grand_total"
            )
        ...
        if self.filters.account_type == "Receivable":
            self.add_column(
                label=_("Customer LPO"), fieldname="po_no", fieldtype="Data"
            )
