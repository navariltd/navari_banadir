...
def prepare_data(data, filters):
    ...
    for row in data:
        ...
        if purchase_order not in totals_dict:
            totals_dict[purchase_order] = {
                "qty": 0.0,
                "received_qty": 0.0,
                "pending_qty": 0.0,
                "custom_container_qty": 0.0,
                "billed_qty": 0.0,
                "qty_to_bill": 0.0,
                "amount": 0.0,
        ...
        totals_dict[purchase_order]["qty"] = row["qty"]
        totals_dict[purchase_order]["received_qty"] = row["received_qty"]
        totals_dict[purchase_order]["pending_qty"] = row["pending_qty"]
        totals_dict[purchase_order]["custom_container_qty"] = row[
            "custom_container_qty"
        ]
        totals_dict[purchase_order]["billed_qty"] = row["billed_qty"]
        totals_dict[purchase_order]["qty_to_bill"] = row["qty_to_bill"]
        totals_dict[purchase_order]["amount"] += row["amount"]
        totals_dict[purchase_order]["received_qty_amount"] += row["received_qty_amount"]
        totals_dict[purchase_order]["billed_amount"] += row["billed_amount"]
        totals_dict[purchase_order]["pending_amount"] += row["pending_amount"]
        totals_dict[purchase_order]["advance_paid"] += row["advance_paid"]
        totals_dict[purchase_order]["balance"] += row["balance"]
    ...
    for purchase_order, totals in totals_dict.items():
        total_row = {
            "date": "",
            "required_by": "",
            "purchase_order": f"Total - {purchase_order}",
            "status": "",
            "supplier": "",
            "project": "",
            "item_code": "",
            ...
            "custom_container_qty": totals["custom_container_qty"],
            "billed_qty": totals["billed_qty"],
            "qty_to_bill": totals["qty_to_bill"],
            "amount": totals["amount"],
            "received_qty_amount": totals["received_qty_amount"],
    ...
    data = sorted(
        ...
        key=lambda x: (
            x["purchase_order"][8:]
            if x["purchase_order"].startswith("Total - ")
            else x["purchase_order"]
        ),
    )
...
def get_columns(filters):
    ...
    columns.extend(
        [
            ...
            {
                "label": (
                    _(f"Amount(<strong>{presentation_currency}</strong>)")
                    if not filters.get("in_party_currency")
                    else _("Amount")
                ),
                "fieldname": "amount",
                "fieldtype": "Currency",
            ...
            {
                "label": (
                    f"Advance Paid (<strong>{presentation_currency}</strong>)"
                    if not filters.get("in_party_currency")
                    else _("Advance Paid")
                ),
            ...
            {
                "label": (
                    f"Balance Amount(<strong>{presentation_currency}</strong>)"
                    if not filters.get("in_party_currency")
                    else _("Balance Amount")
                ),
                "fieldname": "balance",
                "fieldtype": "Currency",
                "options": "currency",
            ...
            {
                "label": (
                    _(f"Billed Amount(<strong>{presentation_currency}</strong>)")
                    if not filters.get("in_party_currency")
                    else _("Billed Amount")
                ),
            ...
            {
                "label": _(
                    f"Pending Amount(<strong>{presentation_currency}</strong>)"
                    if not filters.get("in_party_currency")
                    else "Pending Amount"
                ),
            ...
            {
                "label": (
                    _(f"Received Qty Amount(<strong>{presentation_currency}</strong>)")
                    if not filters.get("in_party_currency")
                    else _("Received Qty Amount")
                ),
...
def convert_currency_columns(data, filters):
    ...
    from_currency = filters.get("presentation_currency") or frappe.get_cached_value(
        "Company", filters.company, "default_currency"
    )
    currency_fields = [
        "amount",
    ...
    for entry in data:
        for field in currency_fields:
            ...
            entry[field] = convert(
                entry.get(field, 0), from_currency, to_currency, date
            )
...
def calculate_advance_paid(purchase_order):
    ...
    if payment_references:
        ...
        advance_paid = (
            sum([amount["base_paid_amount"] for amount in base_paid_amounts]) or 0.0
        )  # Ensures advance_paid is 0 if None
...
def convert_to_party_currency(data, filters):
    for entry in data:
        ...
        entry["received_qty_amount"] = (
            entry["received_qty_amount"] / entry["conversion_rate"]
        )
